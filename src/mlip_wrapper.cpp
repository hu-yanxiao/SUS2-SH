...
const char* MLIP_Wrapper::tagname = {"sus2"};
...
//        else
//            ERROR("Incorrect settings. Neither MLIP nor ab initio model specified");
            ERROR("Incorrect settings. SUS2 potential backend is not activated.");
    }
...
//        Message("No abinitio model is set");
        ...
        Message("abinitio model: pre-trained potential from file");
    ...
    if (enable_learn)
    {
        if (!enable_EFScalc)
            if (enable_select)    // EFS calculation with abinitio potential before training
            {
                if (efs_ignored)
                {
                    logstrm1 << "SUS2: calculating EFS for selected configurations" << std::endl;
                    MLP_LOG(tagname,logstrm1.str()); logstrm1.str("");
                    int cntr=0;
                    for (Configuration& cfg : p_selector->selected_cfgs)
                    {
                        p_abinitio->CalcEFS(cfg);
                        logstrm1 << "SUS2: " << ++cntr << " processed\r";
                        MLP_LOG(tagname,logstrm1.str()); logstrm1.str("");
...
void MLIP_Wrapper::CalcEFS(Configuration & cfg)
{
    ...
    call_count++;
    std::stringstream logstrm1;
    logstrm1 << "SUS2: processed "    << call_count 
//            << "\tAbinitio_calcs "    << abinitio_count
            << "\tmodel_calcs "      << MTP_count
...
void MLIP_Wrapper::CalcE(Configuration & cfg)
{
  ...
  std::stringstream logstrm1;
    logstrm1 << "SUS2: processed " << ++call_count << " configurations" << endl;
  MLP_LOG(tagname,logstrm1.str()); logstrm1.str("");
}
