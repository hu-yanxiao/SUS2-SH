...
void ErrorMonitor::collect(Configuration& cfg_valid, Configuration& cfg_check, double wgt)
{
	...
	atom_count_total += cfg_valid.size();
...
void ErrorMonitor::GetReport(std::string& log)
{
  std::stringstream logstrm1;
  logstrm1 << "summary"
			<< " energy_MAE(meV/atom)=" << energy_mae_mev_per_atom()
			<< " force_MAE(meV/A)=" << force_mae_mev_per_a()
			<< " stress_MAE(eV)=" << stress_mae_ev()
			<< std::endl;
  log = logstrm1.str();
}
...
#ifdef MLIP_MPI
{
	...
	atom_count_total = buffer_errmon.atom_count_total;
