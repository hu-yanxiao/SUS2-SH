...
const char version_str[] = "2026-04-17 developer version";
...
bool Commands(const string& command, vector<string>& args, map<string, string>& opts)
{
...
#endif
				<< "Hu Yanxiao | SUSTech\n"
				<< "Cite: https://doi.org/10.1073/pnas.2503439122\n";
	...
	BEGIN_UNDOCUMENTED_COMMAND("run",
		...
		"mlp-sus2 run settings-file [options]:\n"
		...
		if (args.size() == 0) {
			cout << "mlp-sus2 run: minimum 1 argument required\n";
			return 1;
		}
	...
	BEGIN_COMMAND("relax",
		...
		"mlp-sus2 relax settings-file [options]:\n"
		...
		if (args.size() == 0) {
			cout << "mlp-sus2 relax: minimum 1 argument required\n";
			return 1;
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("make-cell-square",
		...
		"mlp-sus2 make-cell-square in.cfg out.cfg\n"
		"  Options:\n"
		...
		if (opts["min-atoms"] != "") {
			try { min_atoms = stoi(opts["min-atoms"]); }
				...
				cerr << (string)"mlp-sus2 error: " + opts["min-atoms"] + " is not an integer\n";
			}
		}
		if (opts["max-atoms"] != "") {
			try { max_atoms = stoi(opts["max-atoms"]); }
			catch (invalid_argument) {
				cerr << (string)"mlp-sus2 error: " + opts["max-atoms"] + " is not an integer\n";
			}
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("replicate-cell",
		...
		"mlp-sus2 replicate-cell in.cfg out.cfg nx ny nz:\n"
		"or"
		"mlp-sus2 replicate-cell in.cfg out.cfg nxx nxy nxz nyx nyy nyz nzx nzy nzz:\n"
		...
		if (args.size() == 5) {
			...
			try { nx = stoi(args[2]); }
				...
				cerr << (string)"mlp-sus2 error: " + args[2] + " is not an integer\n";
			}
			try { ny = stoi(args[3]); }
			catch (invalid_argument) {
				cerr << (string)"mlp-sus2 error: " + args[3] + " is not an integer\n";
			}
			try { nz = stoi(args[4]); }
			catch (invalid_argument) {
				cerr << (string)"mlp-sus2 error: " + args[4] + " is not an integer\n";
			}
			...
			for (int i = 0; i < 9; i++) {
				try { args_int[i/3][i%3] = stoi(args[i+2]); }
					...
					cerr << (string)"mlp-sus2 error: " + args[i+2] + " is not an integer\n";
				}
			}
	...
	BEGIN_COMMAND("convert-cfg",
		...
		"mlp-sus2 convert-cfg [options] inputfilename outputfilename\n"
		...
		"\n"
		"  <format> can be:\n"
		"  txt (default): SUS2 textual format\n"
		"  bin:           SUS2 binary format\n"
	...
	BEGIN_COMMAND("train",
		...
		"    --trained-pot-name=<string>: filename for trained potential. Default=Trained.mtp_\n"
                "    --distribution-weight=<double>: weight of distribution in fitting. Default=1\n"
		"    --std-weight=<double>: weight of joint std regularization term.\n"
		"    --stdd-weight=<double>: weight of center-std regularization term.\n"
			"    --bfgs-conv-tol=<double>: stop if error dropped by a factor smaller than this\n"
			"                              over 50 BFGS iterations. Default=1e-3\n"
			"    --bfgs-trace-file=<string>: optional CSV trace file for accepted BFGS steps\n"
			"    --bfgs-line-search=<wolfe|armijo-value>: BFGS line search. Default=wolfe\n"
			"    --radial-smooth=<double>: H1 radial smoothness penalty on dR/dr over (0, cutoff).\n"
			"                              Default=1e-6; set 0 to disable.\n"
			"    --radial-smooth-grid=<int>: midpoint quadrature grid for --radial-smooth. Default=128\n"
			"    --scalar-weight-l2-preset=<none|recommended>: scalar-weight L2 preset.\n"
			"        Default=none. recommended sets head=1e-5, gate_scalar=1e-4,\n"
			"        gate_mix=1e-5, gate_full=1e-4.\n"
			"    --scalar-head-l2=<double>: L2 penalty on linear SH scalar head v_q.\n"
		...
		"                           Default=1000\n"
			...
			"    Untrained/incomplete initial models run one implicit rescale+TrainLinear\n"
			"    warmup before BFGS even when --do-lin is not set.\n"
		...
		"                        used for low-strength pairs, the second for high-strength pairs.\n"
		"    --s-range=<a,b>: override the pair-strength interpolation bounds used for the\n"
		"                     second nonlinear scaling parameter. The first value is used for\n"
		"                     low-strength pairs, the second for high-strength pairs.\n"
		...
		if (explicit_inline_sh || implicit_inline_sh) {
			...
			return 1;
                ...
                //if(opts["distribution-weight"] != "") 
                      //  weight_energy = stod(opts["distribution-weight"]);
...
//                        weight_std = stod(opts["std-weight"]);
	...
	BEGIN_COMMAND("calc-errors",
		"compares EFS calculated by the current potential with EFS from the database",
		"mlp-sus2 calc-errors pot.mtp db.cfg:\n"
		"calculates errors of \"pot.mtp\" on the database \"db.cfg\"\n"
	) {

		if (args.size() != 2) {
			cout << "mlp-sus2 calc-errors: 2 arguments required\n";
			return 1;
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("add-from-outcar",
		...
		"mlp-sus2 add-from-outcar db outcar [outcar2 outcar3 ...]:\n"
		"reads one configuration from each of the outcars\n"
		"and APPENDs to the database db\n"
		) {

		if (args.size() < 2) {
			cout << "mlp-sus2 add-from-outcar: minimum 2 arguments required\n";
			return 1;
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("add-from-outcar-last",
		...
		"mlp-sus2 add-from-outcar-last db outcar [outcar2 outcar3 ...]:\n"
		"reads the dynamics from OUTCAR(s) and gets the last configuration from each OUTCAR.\n"
		"APPENDs all last configurations to the database db\n"
		) {

		if (args.size() < 2) {
			std::cout << "mlp-sus2 add-from-outcar: minimum 2 arguments required\n";
			return 1;
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("to-poscar",
		...
		"mlp-sus2 to-poscar db [poscar-file]:\n"
		...
		if (args.size() != 1 && args.size() != 2) {
			cout << "mlp-sus2 to-poscar: 1 or 2 arguments required\n";
			return 1;
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("to-bin",
		...
		"mlp-sus2 to-bin db.cfgs db.bin.cfgs:\n"
		"Reads the database db.cfgs and saves in a binary format to db.bin.cfgs\n"
		) {

		if (args.size() < 3) {
			cout << "mlp-sus2 to-bin: minimum 2 arguments required\n";
			return 1;
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("from-bin",
		...
		"mlp-sus2 from-bin db.bin.cfgs db.cfgs:\n"
	...
	BEGIN_UNDOCUMENTED_COMMAND("fix-lattice",
		...
		"mlp-sus2 fix-lattice in.cfg out.cfg:\n"
		) {
	...
	BEGIN_UNDOCUMENTED_COMMAND("compress-extend",
		...
		"mlp-sus2 compress-extend input.cfg deformed.cfg:\n"
		) {	
	...
	BEGIN_COMMAND("mindist",
        ...
        "mlp-sus2 mindist db.cfg [Options]\n"
        "  Options can be given in any order. Options include:\n"
        "  --no-overwrite: do not change db.cfg, only print global mindist\n"
        ) {

		if (args.size() != 1) {
			cout << "mlp-sus2 mindist: 1 arguments required\n";
			return 1;
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("subsample",
		...
		if (args.size() != 3) {
			cout << "mlp-sus2 subsample: 3 arguments required\n";
			return 1;
		}
	...
	BEGIN_UNDOCUMENTED_COMMAND("fix-cfg",
		...
		"mlp-sus2 fix-db db.cfg [options]\n"
		"  Options:\n"
	...
	BEGIN_UNDOCUMENTED_COMMAND("filter-nonconv",
		...
		"mlp-sus2 filter-nonconv db.cfg\n"
		) {
		if (args.size() != 1) {
		std::cerr << "Error: 1 argument required" << std::endl;
		std::cerr << "Usage: mlp-sus2 filter-nonconv db.cfg" << std::endl;
		return 1;
	}
	std::vector<Configuration> db;
	{
