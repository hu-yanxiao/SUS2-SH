...
tmp_dir=".codex_tmp/sh_two_layer_residual_force_fd"
mkdir -p "$tmp_dir"

init_model="$tmp_dir/residual_init.mtp"
trained_model="$tmp_dir/residual_trained.mtp"
...
replace_list("two_layer_gate_weights", gate_update)
replace_list("two_layer_residual_e0_coeffs", e0_update)
open(out_model, "w").write(text)
...
if err > max(2.0e-6, 5.0e-2 * scale):
    raise SystemExit(
        f"residual two-layer force FD mismatch: force={analytic_force} dE/dx={fd_dedx} err={err}"
    )
PY
