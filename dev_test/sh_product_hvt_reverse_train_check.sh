...
WORKDIR=$(mktemp -d "${TMPDIR:-/tmp}/sus2sh-product-hvt-reverse-XXXXXX")
...
run_train() {
  ...
  (
    cd "$dir"
    SUS2_SH_PRODUCT_ROWS=0 \
    SUS2_SH_PRODUCT_HVT_REVERSE="$mode" \
...
run_train 1 "$WORKDIR/hvt"

python3 - "$WORKDIR/base/current.mtp" "$WORKDIR/hvt/current.mtp" <<'PY'
import re
import sys
...
hvt = values(sys.argv[2])
if len(base) != len(hvt):
    raise SystemExit(f"numeric token count mismatch: {len(base)} vs {len(hvt)}")

max_abs = 0.0
for i, (a, b) in enumerate(zip(base, hvt)):
    diff = abs(a - b)
    max_abs = max(max_abs, diff)
    if diff > 1.0e-10:
        raise SystemExit(f"token {i} differs: {a} vs {b}, diff={diff}")

print(f"SH product HVT reverse train check passed, max_abs={max_abs:.3e}")
PY
